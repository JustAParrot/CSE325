...
public class GameState
{
    ...
    public byte PlayPiece(int column)
    {
        ...
        // W04 Keep tracj of streak
        var result = CheckForWin();
        if (result == WinState.Player1_Wins || result == WinState.Player2_Wins)
        {
            UpdateWinStreaks(result);
        }
    ...
    // Keep track of consecutive wins for each player - W04
    public int Player1WinStreak { get; private set; }
    public int Player2WinStreak { get; private set; }
    private int _lastWinner = 0;

    public void UpdateWinStreaks(WinState result)
    {
        var winner = result switch
        {
            WinState.Player1_Wins => 1,
            WinState.Player2_Wins => 2,
            _ => 0
        };
        ...
        {
            Player1WinStreak = (winner == 1) ? 1 : 0;
            Player2WinStreak = (winner == 2) ? 1 : 0;
            _lastWinner = winner;
            return;
        }

        if (winner == 1) Player1WinStreak++;
        else Player2WinStreak++;
    }
}